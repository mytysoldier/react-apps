...
type SubscriptionResolver interface {
	MessagePosted(ctx context.Context, userID string) (<-chan *model.Message, error)
}
...
func (ec *executionContext) _Subscription_messagePosted(ctx context.Context, field graphql.CollectedField) (ret func(ctx context.Context) graphql.Marshaler) {
	...
	resTmp, err := ec.ResolverMiddleware(ctx, func(rctx context.Context) (interface{}, error) {
		ctx = rctx // use context from middleware stack in children
		return ec.resolvers.Subscription().MessagePosted(rctx, fc.Args["userId"].(string))
	})
	if err != nil {
		ec.Error(ctx, err)
		return nil
	...
	return func(ctx context.Context) graphql.Marshaler {
		...
		case res, ok := <-resTmp.(<-chan *model.Message):
			...
			return graphql.WriterFunc(func(w io.Writer) {
				...
				w.Write([]byte{':'})
				ec.marshalNMessage2ᚖbackendᚋgraphᚋmodelᚐMessage(ctx, field.Selections, res).MarshalGQL(w)
				w.Write([]byte{'}'})
			})
		case <-ctx.Done():
			return nil
